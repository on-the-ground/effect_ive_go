...
// GetFromEffect fetches a typed value from the State effect using the provided key.
...
func GetFromEffect[T any](ctx context.Context, key string) (T, error) {
	return helper.GetTypedValueOf[T](func() (any, error) {
		return Effect(ctx, Load{Key: key})
	})
}

// MustGetFromEffect is the panic-on-failure variant of GetFromStateEffect.
