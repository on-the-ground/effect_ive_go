...
func (sH stateHandler) handle(ctx context.Context, payload StatePayload) (any, error) {
	...
	case SetStatePayload:
		...
		swapped := sH.stateMap.CompareAndSwap(payload.Key, res, payload.Value)

		newForAll := errors.Is(err, ErrKeyNotFound)
		newForLocal := delegated
		localDiff := !newForLocal && !reflect.DeepEqual(res, payload.Value)

		if swapped && (newForAll || newForLocal || localDiff) {
			sH.sink <- statePayloadWithNowTimeSpan(payload)
		}
	...
	case DeleteStatePayload:
		...
		deleted := sH.stateMap.CompareAndDelete(payload.Key, res)
...
func statePayloadWithNowTimeSpan(payload StatePayload) TimeBoundedStatePayload {
